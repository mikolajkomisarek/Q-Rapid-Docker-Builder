...
if(!require(elastic)){
  install.packages("elastic", repos='https://cloud.r-project.org')
  library(elastic)
}
...
elasticConnection <- function(host, path, user, pwd, port) {
  ...
  print(paste(host,path,user,pwd,port, sep="|"))
  print('connect')
  con <<- connect(host = host, path = path, user= user, pwd = pwd,
          port = port, transport_schema  = "http")
  print('ping')
#  con$ping()
  print('condon2')
}
...
searchElement <- function(name, index, tsfrequency, returnDF) {
  ...
  esearch <- Search(conn=con, index = index, q = paste(searchString, name, sep = ''),
